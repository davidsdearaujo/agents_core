...
AgentsCoreConfig _configWithKey({String? apiKey, Logger? logger}) =>
    AgentsCoreConfig(
...
void main() {
  ...
  group(
    ...
    () {
      test(
        'API key config is retained across SocketException retries',
        () async {
      ...
      test(
        'API key config is retained across TimeoutException retries',
        () async {
          const apiKey = 'retry-key-timeout';
          final send = _FakeHttpSend([TimeoutException('timed out'), _ok()]);
          final delay = _RecordingDelay();
  ...
  group('API key regression — real HTTP stack retry verification', () {
    ...
    test(
      'API key is present on sequential requests to different endpoints',
      () async {
    ...
    test('postStream request sends API key on SSE endpoint', () async {
      ...
      final chunks = await client.postStream('/v1/chat/completions', {
        'model': 'test',
        'messages': [
          {'role': 'user', 'content': 'hi'},
        ],
  ...
  group('API key regression — security: no key leakage in exceptions', () {
    test(
      'LmStudioApiException toString() does not contain the API key',
      () async {
        const apiKey = 'super-secret-key-do-not-leak';
        final send = _FakeHttpSend([
          _status(401, '{"error":{"type":"unauthorized","message":"bad key"}}'),
        ]);
    ...
    test(
      'LmStudioConnectionException toString() does not contain the API key',
      () async {
        const apiKey = 'another-secret-key';
        final send = _FakeHttpSend([const SocketException('refused')]);
        final delay = _RecordingDelay();
  ...
  group('API key regression — retry exhaustion', () {
    test('all retries exhausted with API key configured still throws '
    ...
    test('maxRetries=0 with API key configured fails on first error', () async {
      ...
      expect(
        delay.delays,
        isEmpty,
        reason: 'No backoff delay with maxRetries=0',
      );
      client.dispose();
    });
  });
  ...
  // ---------------------------------------------------------------------------
  group(
    'API key regression — non-retryable 4xx errors are immediate failures',
    () {
      test('401 Unauthorized is not retried, even with API key', () async {
        ...
        expect(send.callCount, equals(1), reason: '401 must not trigger retry');
        expect(delay.delays, isEmpty, reason: 'no backoff delay for 401');
        client.dispose();
      });
      ...
      test('403 Forbidden is not retried, even with API key', () async {
        ...
        expect(send.callCount, equals(1), reason: '403 must not trigger retry');
        client.dispose();
      });
      ...
      test('404 Not Found is not retried, even with API key', () async {
        ...
        expect(send.callCount, equals(1), reason: '404 must not trigger retry');
        client.dispose();
      });
    },
  );
