...
class _FakeAgent extends Agent {
  ...
  /// Convenience: always throw a fixed error.
  ...
  for (var i = 0; i < iterationCount; i++) {
    iterations.add(
      AgentLoopIteration(
        ...
        reviewerResult: const AgentResult(
          output: 'reviewed',
          tokensUsed: 5,
        ),
      ),
...
void main() {
  ...
  group('AgentLoopStep construction', () {
    test('stores producer agent reference', () {
      final producer = _FakeAgent(name: 'producer');
    ...
    test('stores static String taskPrompt', () {
      ...
      expect(step.taskPrompt, 'implement feature X');
    });

    test('AgentLoopStep.dynamic stores function taskPrompt', () {
  ...
  group('AgentLoopStepResult', () {
    ...
    test('output returns the LAST iteration producer output when multi-iteration', () {
  ...
  group('Orchestrator + AgentLoopStep integration', () {
    ...
    test('single AgentLoopStep executes producer/reviewer loop and returns AgentLoopStepResult',
        () async {
    ...
    test('AgentStep → AgentLoopStep → AgentStep executes in order with correct types',
        () async {
      final agent1 = _FakeAgent.single(
        name: 'a1',
      ...
      final agent3 = _FakeAgent.single(
        name: 'a3',
    ...
    test('exception in AgentLoopStep is captured when policy is continueOnError', () async {
      final producer = _FakeAgent.throwing(
        name: 'producer',
        error: Exception('loop error'),
      );
      final reviewer = _FakeAgent(name: 'reviewer');
      final afterAgent = _FakeAgent.single(
        name: 'after',
    ...
    test('subsequent steps execute after AgentLoopStep failure with continueOnError',
        () async {
      final producer = _FakeAgent.throwing(
        name: 'producer',
        error: Exception('loop error'),
      );
      final afterAgent = _FakeAgent.single(name: 'after');
    ...
    test('orchestrator FileContext is passed to producer and reviewer', () async {
      final producer = _FakeAgent.single(
        name: 'producer',
    ...
    test('maxIterations from AgentLoopStep limits the number of loop iterations',
        () async {
    ...
    test('isAccepted from AgentLoopStep controls when the loop terminates early',
        () async {
    ...
    test('custom buildProducerPrompt from AgentLoopStep is forwarded to AgentLoop',
        () async {
    ...
    test('custom buildReviewerPrompt from AgentLoopStep is forwarded to AgentLoop',
        () async {
  ...
  group('OrchestratorResult — StepResult type hierarchy', () {
    ...
    test('AgentStep execution produces AgentStepResult in stepResults', () async {
      final orch = Orchestrator(
        context: ctx,
    ...
    test('AgentStepResult.agentResult holds the original AgentResult', () async {
      ...
      final orch = Orchestrator(
        ...
        steps: [
          AgentStep(
            agent: _FakeAgent.single(result: agentResult),
            taskPrompt: 'task',
          ),
        ],
      );
    ...
    test('common StepResult.output accessor works for AgentStep results', () async {
      final r1 = const AgentResult(output: 'out1', tokensUsed: 10);
      final r2 = const AgentResult(output: 'out2', tokensUsed: 20);
      final orch = Orchestrator(
        context: ctx,
        steps: [
          AgentStep(agent: _FakeAgent.single(result: r1), taskPrompt: 'task1'),
          AgentStep(agent: _FakeAgent.single(result: r2), taskPrompt: 'task2'),
        ],
      );
      final result = await orch.run();
    ...
    test('common StepResult.output accessor works for AgentLoopStep results', () async {
      final producer = _FakeAgent.single(
        name: 'producer',
    ...
    test('mixed pipeline stepResults contains correct StepResult subtypes at each index',
        () async {
